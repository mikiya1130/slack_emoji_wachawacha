...
class TestSlackHandler:
    ...
    def mock_emoji_service(self):
        """EmojiServiceのモック"""
        service = AsyncMock()
        service.find_similar_emojis = AsyncMock(return_value=[
    ...
    def test_slack_handler_initialization(self, mock_openai_service, mock_emoji_service):
        """SlackHandlerの初期化テスト"""
    ...
    @pytest.mark.asyncio
    async def test_handle_message_with_valid_message(self, slack_handler, sample_slack_message):
        """有効なメッセージの処理テスト"""
    ...
    @pytest.mark.asyncio
    async def test_handle_message_integration_flow(self, slack_handler, sample_slack_message):
        """メッセージ処理の統合フローテスト"""
        # メッセージ処理の実行
        await slack_handler.handle_message(sample_slack_message)
...
class TestSlackHandlerReactionFeatures:
    ...
    @pytest.fixture
    def slack_handler_with_retry_capability(self, mock_openai_service, mock_emoji_service):
        """リトライ機能を持つSlackHandler（実装されていないため失敗するはず）"""
        from app.services.slack_handler import SlackHandler
        return SlackHandler(mock_openai_service, mock_emoji_service)
    
    @pytest.mark.asyncio
    async def test_add_reactions_with_retry_logic(self, slack_handler_with_retry_capability):
        ...
        assert handler.app.client.reactions_add.call_count == 3, \
            "Retry logic should call reactions_add 3 times (2 failures + 1 success)"
    
    @pytest.mark.asyncio
    async def test_add_reactions_with_exponential_backoff(self, slack_handler_with_retry_capability):
    ...
    @pytest.mark.asyncio
    async def test_add_reactions_max_retry_limit(self, slack_handler_with_retry_capability):
    ...
    @pytest.mark.asyncio
    async def test_add_reactions_concurrent_processing(self, slack_handler_with_retry_capability):
        ...
        assert elapsed_time < 0.3, \
            f"Concurrent processing should complete faster than sequential (took {elapsed_time:.2f}s)"
    
    @pytest.mark.asyncio
    async def test_add_reactions_with_rate_limit_monitoring(self, slack_handler_with_retry_capability):
        ...
        assert hasattr(handler, 'rate_limit_info'), \
...
class TestSlackHandlerErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_handle_message_with_openai_error(self, slack_handler_with_error_services, sample_slack_message):
    ...
    @pytest.mark.asyncio
    async def test_handle_message_with_emoji_service_error(self, slack_handler_with_error_services, sample_slack_message):
