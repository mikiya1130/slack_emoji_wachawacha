...
class TestEmojiServiceInitialization:
    ...
    def test_emoji_service_initialization_with_cache(self):
        ...
        assert emoji_service.cache_enabled == True
...
class TestEmojiServiceBasicOperations:
    ...
    async def test_save_emoji_success(self, mock_emoji_service, sample_emoji_data):
        ...
        mock_emoji_service.database_service.insert_emoji.return_value = sample_emoji_data
        
        result = await mock_emoji_service.save_emoji(sample_emoji_data)
        
        assert result == sample_emoji_data
        mock_emoji_service.database_service.insert_emoji.assert_called_once_with(sample_emoji_data)
    
    @pytest.mark.asyncio
    async def test_get_emoji_by_id_success(self, mock_emoji_service, sample_emoji_data):
        """ID指定での絵文字取得テスト"""
        # DatabaseServiceのmockを設定
        mock_emoji_service.database_service.get_emoji_by_id.return_value = sample_emoji_data
    ...
    @pytest.mark.asyncio
    async def test_get_emoji_by_code_success(self, mock_emoji_service, sample_emoji_data):
        """コード指定での絵文字取得テスト"""
        # DatabaseServiceのmockを設定
        mock_emoji_service.database_service.get_emoji_by_code.return_value = sample_emoji_data
        
        result = await mock_emoji_service.get_emoji_by_code(":test_emoji:")
        
        assert result == sample_emoji_data
        mock_emoji_service.database_service.get_emoji_by_code.assert_called_once_with(":test_emoji:")
    ...
    async def test_update_emoji_success(self, mock_emoji_service, sample_emoji_data):
        ...
        # DatabaseServiceのmockを設定
        mock_emoji_service.database_service.update_emoji.return_value = sample_emoji_data
        
        result = await mock_emoji_service.update_emoji(sample_emoji_data)
        
        assert result == sample_emoji_data
        mock_emoji_service.database_service.update_emoji.assert_called_once_with(sample_emoji_data)
    ...
    async def test_get_all_emojis_with_pagination(self, mock_emoji_service):
        """ページネーション付き全絵文字取得テスト"""
        sample_emojis = [
            EmojiData(code=f":emoji_{i}:", description=f"Emoji {i}")
            for i in range(5)
        ]
        ...
        mock_emoji_service.database_service.get_all_emojis.assert_called_once_with(limit=10, offset=0)
...
class TestEmojiServiceVectorOperations:
    ...
    async def test_find_similar_emojis_by_vector(self, mock_emoji_service):
        ...
        # DatabaseServiceのmockを設定
        mock_emoji_service.database_service.find_similar_emojis.return_value = expected_emojis
    ...
    async def test_find_similar_emojis_with_filters(self, mock_emoji_service):
        ...
        expected_emojis = [
            EmojiData(code=":smile:", description="Happy face", emotion_tone="positive", category="emotions")
        ]
        ...
        result = await mock_emoji_service.find_similar_emojis(query_vector, limit=3, filters=filters)
    ...
    async def test_find_similar_emojis_by_text(self, mock_emoji_service):
        ...
        result = await mock_emoji_service.find_similar_emojis_by_text(query_text, limit=2)
...
class TestEmojiServiceBulkOperations:
    ...
    @pytest.mark.asyncio
    async def test_load_emojis_from_json_file(self, mock_emoji_service, sample_emoji_json_data):
    ...
    async def test_bulk_save_emojis(self, mock_emoji_service, sample_emoji_json_data):
        ...
        # DatabaseServiceのmockを設定
        mock_emoji_service.database_service.batch_insert_emojis.return_value = emoji_list
        
        result = await mock_emoji_service.bulk_save_emojis(emoji_list)
        
        assert result == emoji_list
        mock_emoji_service.database_service.batch_insert_emojis.assert_called_once_with(emoji_list)
    
    @pytest.mark.asyncio
    async def test_load_and_save_emojis_from_json(self, mock_emoji_service, sample_emoji_json_data):
        ...
        try:
            ...
            # DatabaseServiceのmockを設定
            mock_emoji_service.database_service.batch_insert_emojis.return_value = expected_emojis
            
            # 読み込みと保存を一度に実行
            result = await mock_emoji_service.load_and_save_emojis_from_json(json_file_path)
...
class TestEmojiServiceCaching:
    ...
    async def test_get_emoji_with_cache_miss(self, mock_emoji_service_with_cache):
        ...
        # DatabaseServiceのmockを設定
        mock_emoji_service_with_cache.database_service.get_emoji_by_id.return_value = sample_emoji
        
        # 初回はキャッシュミス
        result = await mock_emoji_service_with_cache.get_emoji_by_id(1)
        
        assert result == sample_emoji
        mock_emoji_service_with_cache.database_service.get_emoji_by_id.assert_called_once_with(1)
    ...
    async def test_get_emoji_with_cache_hit(self, mock_emoji_service_with_cache):
        ...
        # データベースは1回だけ呼び出される
        assert mock_emoji_service_with_cache.database_service.get_emoji_by_id.call_count == 1
    ...
    async def test_cache_invalidation_on_update(self, mock_emoji_service_with_cache):
        ...
        # DatabaseServiceのmockを設定
        mock_emoji_service_with_cache.database_service.get_emoji_by_id.return_value = sample_emoji
        mock_emoji_service_with_cache.database_service.update_emoji.return_value = updated_emoji
        ...
        mock_emoji_service_with_cache.database_service.get_emoji_by_id.return_value = updated_emoji
...
class TestEmojiServiceErrorHandling:
    ...
    async def test_handle_database_connection_error(self, mock_emoji_service):
        ...
        mock_emoji_service.database_service.get_emoji_by_id.side_effect = DatabaseConnectionError("Connection failed")
        ...
        assert "connection" in str(excinfo.value).lower() or "database" in str(excinfo.value).lower()
    ...
    async def test_handle_file_not_found(self, mock_emoji_service):
        ...
        assert "not found" in str(excinfo.value).lower() or "file" in str(excinfo.value).lower()
...
def sample_emoji_list():
    """テスト用絵文字リスト"""
    return [
        EmojiData(code=":smile:", description="Smiling face", category="emotions", emotion_tone="positive"),
        EmojiData(code=":frown:", description="Frowning face", category="emotions", emotion_tone="negative"),
        EmojiData(code=":thumbsup:", description="Thumbs up", category="gestures", emotion_tone="positive"),
        EmojiData(code=":heart:", description="Red heart", category="symbols", emotion_tone="positive"),
        EmojiData(code=":fire:", description="Fire", category="objects", emotion_tone="neutral")
    ]
