...
class SlackHandler:
    ...
    async def add_reactions(self, channel: str, timestamp: str, emojis: List[str]) -> None:
        ...
        for emoji in emojis:
            ...
            if emoji_name:  # 有効な絵文字名のみ処理
                task = self._add_single_reaction_with_retry(channel, timestamp, emoji_name)
                tasks.append(task)
    ...
    async def _add_single_reaction_with_retry(self, channel: str, timestamp: str, emoji_name: str) -> None:
        """
        ...
        for attempt in range(self.max_retries + 1):  # 初回 + リトライ
            try:
                ...
                # 成功ログ（リトライした場合のみ詳細表示）
                if attempt > 0:
                    logger.info(f"Successfully added reaction {emoji_name} after {attempt} retries")
                    self.reaction_metrics["retries_performed"] += attempt
                else:
                    logger.debug(f"Added reaction {emoji_name} to {channel}:{timestamp}")
                ...
                if self._is_retryable_error(error_msg) and attempt < self.max_retries:
                    ...
                    if attempt == self.max_retries:
                        logger.error(
                            f"Max retries ({self.max_retries}) exceeded for {emoji_name}: {error_msg}"
                        )
                    else:
                        logger.error(f"Non-retryable error for {emoji_name}: {error_msg}")
